...
<body>
  ...
  <main>
    <section class="filters" id="filters">
      ...
      <!-- NUEVOS filtros por cuotas -->
      <label>Venc. cuota desde
        <input type="date" name="cuota_venc_desde">
      </label>
      <label>Venc. cuota hasta
        <input type="date" name="cuota_venc_hasta">
      </label>

      <div class="filters-actions">
        <span class="checkbox">
          <input type="checkbox" id="solo-vencidas" name="solo_vencidas">
          <label for="solo-vencidas">Solo vencidas (madre)</label>
        </span>
        <span class="checkbox">
          <input type="checkbox" id="solo-vencidas-cuotas" name="solo_vencidas_cuotas">
          <label for="solo-vencidas-cuotas">Solo vencidas (cuotas)</label>
        </span>
        <span class="checkbox">
      ...
      <table id="table-cxp">
        <thead>
          <tr>
            ...
            <th>Total</th>
            <th>Saldo</th>
            <!-- NUEVO: KPIs cuotas en la grilla -->
            <th>Cuotas (KPI)</th>
            <th>Órdenes</th>
            <th>Estado</th>
            <th>Acciones</th>
  ...
  <div class="modal-backdrop" id="modal">
    <div class="modal">
      ...
      <div class="content">
        ...
        <!-- NUEVO: sección de cuotas -->
        <div class="section" id="cuotas-section">
          <h3>Plan de cuotas</h3>
          <div id="cuotas-kpis" style="margin-bottom:8px"></div>
          <table class="subtable" id="cuotas-table">
            <thead>
              <tr>
                <th>#</th>
                <th>Vencimiento</th>
          ...
          <div id="cuotas-empty" class="empty" style="display:none;">Sin cuotas registradas.</div>
        </div>
      ...
      <form id="op-form">
        ...
        <div class="content">
          <div class="detail-grid" id="op-meta">
            <div class="detail-card"><strong>Proveedor</strong><span id="op-provider">-</span></div>
            <div class="detail-card"><strong>RUC</strong><span id="op-provider-ruc">-</span></div>
            <div class="detail-card"><strong>Moneda</strong><span id="op-moneda">-</span></div>
            <div class="detail-card"><strong>Facturas</strong><span id="op-count">0</span></div>
          </div>
              ...
              <thead>
                <tr>
                  <th style="text-align:center;">
                    <input type="checkbox" id="op-pay-all" title="Seleccionar todas">
                  </th>
  ...
  <script>
    ...
    const cuotasKpisBox = document.getElementById('cuotas-kpis');
    const cuotasTableBody = document.querySelector('#cuotas-table tbody');
    ...
    function formatCurrency(amount, currency = '') {
      const value = Number(amount || 0).toLocaleString('es-PY', {
    ...
    function renderCuotasKpiInline(cuotas) {
      if (!cuotas) return '';
      const chips = [];
      chips.push(`<span class="kpi-chip">Tot: ${cuotas.total || 0}</span>`);
      chips.push(`<span class="kpi-chip">Abiertas: ${cuotas.abiertas || 0}</span>`);
      chips.push(`<span class="kpi-chip">Saldo: ${formatCurrency(cuotas.saldo_abiertas || 0)}</span>`);
      if (cuotas.proximo_venc) chips.push(`<span class="kpi-chip">Próx: ${cuotas.proximo_venc}</span>`);
      if ((cuotas.monto_vencido || 0) > 0) chips.push(`<span class="kpi-chip">Vencido: ${formatCurrency(cuotas.monto_vencido)}</span>`);
      return chips.join('');
    }

    function renderOrdenResumen(op, moneda) {
      if (!op || op.total === 0) return '<span class="order-box">Sin órdenes</span>';
      const partes = [];
      partes.push(`<strong>${op.total} orden${op.total > 1 ? 'es' : ''}</strong>`);
      if (op.monto_reservado > 0) partes.push(`Reservado: ${formatCurrency(op.monto_reservado, moneda)}`);
      if (op.monto_total > op.monto_reservado) partes.push(`Aplicado: ${formatCurrency(op.monto_total - op.monto_reservado, moneda)}`);
      if (op.ultima_op) partes.push(`Última: #${op.ultima_op} (${escapeHtml(op.ultimo_estado || '-')})`);
    ...
    function renderSummary(summary, show) {
      ...
      const items = [{
          ...
          value: summary.saldo_por_pagar
        },
        {
          label: 'Saldo pendiente',
          value: summary.saldo_pendiente
        },
        {
          label: 'Saldo parcial',
          value: summary.saldo_parcial
        },
        {
          label: 'Saldo vencido',
          value: summary.saldo_vencido
        },
        {
          label: 'Total facturado',
          value: summary.total_facturado
        }
      ];
      if (summary.saldo_reservado_op !== undefined) items.push({
        label: 'Saldo reservado (OP)',
        value: summary.saldo_reservado_op
      });
      summaryBox.innerHTML = items.map(it => `
      <div class="summary-item"><span class="label">${it.label}</span><span class="value">${formatCurrency(it.value)}</span></div>
    `).join('');
    ...
    function clearMissingSelections() {
      for (const key of Array.from(selectedRows.keys())) {
        if (!rowsCache.has(key)) selectedRows.delete(key);
      }
    }
    ...
    function updateSelectionUI() {
      ...
      const btnOpen = btnOpenOp,
        ...
        infoEl = selectionInfo;

      if (!selection.length) {
        infoEl.textContent = 'No hay facturas seleccionadas.';
      ...
      const totalSaldo = selection.reduce((s, r) => s + (r.saldo_actual || 0), 0);
      const infoText = `Seleccionadas ${selection.length} · Proveedor: ${escapeHtml(selection[0].proveedor.nombre)} · Moneda: ${selection[0].moneda} · Saldo total: ${formatCurrency(totalSaldo, selection[0].moneda)}`;
      ...
      if (!valid) {
        infoEl.textContent = `${infoText} · ${errorMsg}`;
    ...
    function renderTable(rows) {
      ...
      rows.forEach(r => {
        rowsCache.set(r.id_cxp, r);
        if (selectedRows.has(r.id_cxp)) selectedRows.set(r.id_cxp, r);
      });

      rows.forEach(row => {
        const hasOpenOp = row.orden_pago && row.orden_pago.abiertas > 0;
        if (hasOpenOp && selectedRows.has(row.id_cxp)) selectedRows.delete(row.id_cxp);
        ...
        const cuotasHtml = renderCuotasKpiInline(row.cuotas);

        tr.innerHTML = `
        <td><input type="checkbox" class="row-select" data-id="${row.id_cxp}" ${isSelected ? 'checked' : ''} ${hasOpenOp ? 'disabled' : ''}></td>
        <td><div style="font-weight:600;">${escapeHtml(row.proveedor.nombre)}</div><div style="font-size:12px;color:#64748b;">RUC: ${escapeHtml(row.proveedor.ruc || '-')}</div></td>
        <td><div>${escapeHtml(row.documento.numero)}</div>${row.documento.timbrado ? `<div style="font-size:12px;color:#64748b;">Timbrado ${escapeHtml(row.documento.timbrado)}</div>` : ''}</td>
    ...
    document.getElementById('btn-export').addEventListener('click', async () => {
      ...
      try {
        ...
        const header = ['Proveedor', 'RUC', 'Documento', 'Emisión', 'Vencimiento', 'Condición', 'Total', 'Saldo', 'Estado', 'Cuotas abiertas', 'Saldo cuotas', 'Próx venc', 'Monto vencido', 'Órdenes abiertas', 'Monto reservado'];
        ...
        rows.forEach(r => {
          csvRows.push([
            ...
            r.fecha_emision, r.fecha_venc, r.condicion || '',
            r.total_factura, r.saldo_actual, r.estado,
            r.cuotas ? r.cuotas.abiertas : 0,
            r.cuotas ? r.cuotas.saldo_abiertas : 0,
            r.cuotas ? (r.cuotas.proximo_venc || '') : '',
            r.cuotas ? (r.cuotas.monto_vencido || 0) : 0,
    ...
    function renderDetail(cxp, ordenes) {
      ...
      const infoCuotas = cxp.cuotas_info || null;
    ...
    `;
      ...
      // KPIs cuotas en el modal
      cuotasKpisBox.innerHTML = infoCuotas ? `
      <span class="kpi-chip">Cuotas: ${infoCuotas.total}</span>
      <span class="kpi-chip">Abiertas: ${infoCuotas.abiertas}</span>
      <span class="kpi-chip">Saldo abiertas: ${formatCurrency(infoCuotas.saldo_abiertas, cxp.moneda)}</span>
      ${infoCuotas.proximo_venc ? `<span class="kpi-chip">Próximo venc: ${infoCuotas.proximo_venc}</span>`:''}
      ${infoCuotas.monto_vencido>0 ? `<span class="kpi-chip">Vencido: ${formatCurrency(infoCuotas.monto_vencido, cxp.moneda)}</span>`:''}
    ` : '';
      ...
      if (!cuotas.length) {
        cuotasEmpty.style.display = 'block';
      } else {
        cuotasEmpty.style.display = 'none';
        cuotas.forEach(c => {
          const tr = document.createElement('tr');
          const cls = (c.estado === 'Pendiente' || c.estado === 'Parcial') && new Date(c.vencimiento) < new Date() ? 'style="color:#b91c1c;font-weight:600;"' : '';
          tr.innerHTML = `
          <td>${c.nro}</td>
          <td ${cls}>${c.vencimiento}</td>
          <td style="text-align:right;">${formatCurrency(c.monto, cxp.moneda)}</td>
          <td style="text-align:right;font-weight:600;">${formatCurrency(c.saldo, cxp.moneda)}</td>
          <td>${escapeHtml(c.estado)}</td>
          <td>${escapeHtml(c.obs||'')}</td>`;
          cuotasTableBody.appendChild(tr);
        });
      }
    }
    ...
    function renderOrdenes(ordenes, moneda) {
      ...
      ordenes.forEach(op => {
        ...
        const chequeTxt = op.cheque ? `Cheque #${escapeHtml(op.cheque.numero||'')} (${escapeHtml(op.cheque.estado)})` : 'Sin cheque';
        const tr = document.createElement('tr');
        tr.innerHTML = `
    ...
    async function openDetail(id) {
      try {
        const res = await fetch(`${apiUrl}?id_cxp=${id}`, {
          credentials: 'same-origin'
        });
        const json = await res.json();
    ...
    function recalcOpTotals() {
      // Solo inputs habilitados (cuotas marcadas)
      const inputs = Array.from(document.querySelectorAll('.op-amount')).filter(i => !i.disabled);
      let total = 0;
    ...
    function updateAccountInfo() {
      ...
      const txt = `Banco: ${cuenta.banco} · Nº ${cuenta.numero_cuenta} · Moneda: ${cuenta.moneda}
Saldo contable: ${formatCurrency(cuenta.saldo_contable,cuenta.moneda)} · Reservado: ${formatCurrency(cuenta.saldo_reservado,cuenta.moneda)} · Disponible: ${formatCurrency(cuenta.saldo_disponible,cuenta.moneda)}`;
      opAccountInfo.textContent = txt;
      opAccountInfo.className = sameCurrency ? 'info-box' : 'info-box warn';
      if (!sameCurrency) opAccountInfo.textContent += ' · Atención: moneda distinta a la de las facturas.';
      recalcOpTotals();
    }
    ...
    function populateAccountSelect(moneda) {
      ...
      matching.concat(others).forEach(acc => {
        ...
        opt.value = acc.id_cuenta_bancaria;
        opt.textContent = `${acc.banco} · ${acc.numero_cuenta} (${acc.moneda})${matching.includes(acc)?' - Moneda coincidente':''}`;
        opAccountSelect.appendChild(opt);
      });
      if (matching.length) opAccountSelect.value = matching[0].id_cuenta_bancaria;
      else if (bankAccounts.length) opAccountSelect.value = bankAccounts[0].id_cuenta_bancaria;
      else opAccountSelect.value = '';
    }

    async function fetchCxpDetail(id) {
      const res = await fetch(`${apiUrl}?id_cxp=${id}`, {
        credentials: 'same-origin'
      });
      const json = await res.json();
      if (!json.ok) throw new Error(json.error || 'No se pudo cargar el detalle');
      return json.cxp; // incluye cuotas y cuotas_info
    }

    // Sincroniza el estado del master checkbox (si existe)
    function syncPayAllCheckbox() {
      const payAll = document.getElementById('op-pay-all');
      if (!payAll) return;
      const checks = opFacturasTableBody.querySelectorAll('.pay-check');
      const all = Array.from(checks);
      const marked = all.filter(c => c.checked);
      if (!all.length) {
        payAll.checked = false;
        payAll.indeterminate = false;
        return;
      }
      payAll.checked = marked.length === all.length && marked.length > 0;
      payAll.indeterminate = marked.length > 0 && marked.length < all.length;
    }

    async function openOpModal(rows) {
      if (!rows.length) {
      ...
      if (rows.some(r => r.proveedor.id !== providerId)) {
        alert('Seleccioná facturas del mismo proveedor.');
        return;
      }
      ...
      // Traemos detalle con cuotas para cada CXP seleccionada
      const detalles = await Promise.all(rows.map(r => fetchCxpDetail(r.id_cxp)));
      ...
      // Construimos filas por CUOTA (no por factura)
      detalles.forEach(det => {
        const docLabel = `${det.documento.numero} (CxP #${det.id_cxp})`;
        const cuotas = det.cuotas || [];
        if (!cuotas.length) {
          const tr = document.createElement('tr');
          tr.innerHTML = `
          <td colspan="5">
            <div style="font-weight:600;">${escapeHtml(docLabel)}</div>
            <div style="font-size:12px;color:#64748b;">Sin cuotas</div>
          </td>`;
        ...
        cuotas.forEach(c => {
          if (!['Pendiente', 'Parcial'].includes(c.estado) || (c.saldo || 0) <= 0) return; // solo cuotas con saldo
          const tr = document.createElement('tr');
          tr.innerHTML = `
          <td style="text-align:center;">
            <input type="checkbox"
                   class="pay-check"
                   data-id-cxp="${det.id_cxp}"
                   data-id-cxp-det="${c.id_cxp_det}"
                   data-max="${c.saldo}">
          </td>
          <td>
            <div style="font-weight:600;">${escapeHtml(docLabel)}</div>
            <div style="font-size:12px;color:#64748b;">Cuota #${c.nro} · id:${c.id_cxp_det}</div>
          </td>
          <td>${c.vencimiento}</td>
          <td style="text-align:right;">${formatCurrency(c.saldo, det.moneda)}</td>
          <td style="text-align:right;">
            <input type="number"
                   class="op-amount"
                   data-id-cxp="${det.id_cxp}"
                   data-id-cxp-det="${c.id_cxp_det}"
                   min="0.01" step="0.01" max="${c.saldo}"
                   value=""
                   disabled
                   style="width:120px;">
          </td>`;
      ...
      // Listeners: activar/desactivar inputs por checkbox
      opFacturasTableBody.querySelectorAll('.pay-check').forEach(chk => {
        chk.addEventListener('change', () => {
          const row = chk.closest('tr');
          const input = row.querySelector('.op-amount');
          const max = parseFloat(chk.dataset.max || '0') || 0;
          if (chk.checked) {
            input.disabled = false;
            input.value = max > 0 ? max.toFixed(2) : '';
          } else {
            input.value = '';
            input.disabled = true;
          }
      ...
      // Master "Seleccionar todas" (si existe en tu thead)
      const payAll = document.getElementById('op-pay-all');
      if (payAll) {
        payAll.checked = false;
        payAll.indeterminate = false;
        payAll.onchange = () => {
          const checks = opFacturasTableBody.querySelectorAll('.pay-check');
          checks.forEach(chk => {
            chk.checked = payAll.checked;
            chk.dispatchEvent(new Event('change'));
          });
        };
      }

      // Cuentas bancarias y totales
    ...
    async function handleOpSubmit(e) {
      ...
      // Solo cuotas marcadas (inputs habilitados)
      const cuotasSel = Array.from(document.querySelectorAll('.op-amount'))
        .filter(i => !i.disabled)
        .map(input => ({
      ...
      if (!cuotasSel.length) {
        opError.textContent = 'Marcá al menos una cuota y asigná un monto mayor a cero.';
        return;
      }

      // Agregado por CXP (compatibilidad)
      const porCxp = new Map();
      cuotasSel.forEach(c => {
        porCxp.set(c.id_cxp, (porCxp.get(c.id_cxp) || 0) + c.monto);
      });
      const facturas = Array.from(porCxp.entries()).map(([id_cxp, monto]) => ({
        id_cxp,
        monto
      }));
      ...
      const total = facturas.reduce((sum, it) => sum + it.monto, 0);
      ...
      const payload = {
        ...
        facturas, // [{id_cxp, monto}] (compat)
        cuotas: cuotasSel, // [{id_cxp, id_cxp_det, monto}] granular
    ...
    tbody.addEventListener('click', e => {
      ...
      if (payBtn) {
        ...
        const cb = tbody.querySelector(`.row-select[data-id="${row.id_cxp}"]`);
    ...
    const btnOpenOpEl = document.getElementById('btn-open-op');
    btnOpenOpEl.addEventListener('click', () => {
    ...
    opModal.addEventListener('click', e => {
      if (e.target === opModal) closeOpModal();
    });
    ...
    // init
    loadBankAccounts().catch(console.error);
    loadData();
  </script>
