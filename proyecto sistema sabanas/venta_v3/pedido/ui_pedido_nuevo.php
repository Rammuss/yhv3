...
<body>
  ...
  <script>
    ...
    /* ===== Helpers IVA ===== */
    function normalizeIvaCode(raw) {
      const t = (raw ?? '').toString().trim().toUpperCase();
      if (t === 'IVA10' || t === 'IVA 10' || t === '10%' || t === '10' || t.includes('10')) return 'IVA10';
      if (t === 'IVA5' || t === 'IVA 5' || t === '5%' || t === '5' || t.includes('5')) return 'IVA5';
      return 'EXE';
    }

    function ivaRate(tipo) {
      const code = normalizeIvaCode(tipo);
      if (code === 'IVA10') return 0.10;
      if (code === 'IVA5') return 0.05;
      return 0.0;
    }

    function ivaLabel(tipo) {
      const code = normalizeIvaCode(tipo);
      if (code === 'IVA10') return '10%';
      if (code === 'IVA5') return '5%';
      return 'Exento';
    }
    ...
    function render() {
      ...
      items.forEach((it, idx) => {
        const rate = ivaRate(it.tipo_iva);
        const importeBruto = it.cantidad * it.precio_unitario; // precio con IVA
        const desc = Math.min(it.descuento || 0, importeBruto);
        const importeFinal = Math.max(0, importeBruto - desc); // precio con IVA - descuento

        let base = importeFinal;
        let iva = 0;
        if (rate > 0) {
          base = importeFinal / (1 + rate);
          iva = importeFinal - base;
        }
    ...
    `;
      ...
      <input type="hidden" name="${baseName}[tipo_iva]" value="${normalizeIvaCode(it.tipo_iva)}">
    `);
      });
    }
    ...
    function agregarItem(prod) {
      const ivaCode = normalizeIvaCode(prod.tipo_iva);
      ...
      prodTimer = setTimeout(async () => {
        ...
        if (js.success && js.productos.length) {
          js.productos.forEach(p => {
            const ivaLbl = ivaLabel(p.tipo_iva);
          ...
          <td><button type="button" class="btn btn-sm" onclick='agregarItem(${data})'>+</button></td>
        `;
    ...
    formPedido.addEventListener('submit', async (e) => {
      ...
      if (!idCliente.value) {
        showToast('Seleccioná un cliente', 'err');
        return;
      }
      if (items.length === 0) {
        showToast('Agregá al menos un ítem', 'err');
        return;
      }
      for (const it of items) {
        if (it.cantidad < 1 || it.cantidad > it.stock_disponible) {
          showToast('Cantidad inválida para ' + it.nombre, 'err');
          return;
        }
      }
