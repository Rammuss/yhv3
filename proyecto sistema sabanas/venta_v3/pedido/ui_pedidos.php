...
<div class="wrap">
  ...
  <div class="card">
    ...
    <form class="row" method="get" autocomplete="off">
      <div style="flex:0 0 100px;"><label>ID Pedido</label><input type="number" name="id_pedido" value="<?= e($id_pedido ?: '') ?>"></div>
      <div style="flex:0 0 150px;"><label>Estado</label>
        <select name="estado">
      ...
      <div style="flex:1;min-width:220px;"><label>Cliente / CI-RUC</label><input type="text" name="q" value="<?= e($q) ?>" placeholder="Nombre, apellido o CI/RUC"></div>
  ...
  <div class="card">
    <div class="muted" style="margin-bottom:8px">
      Resultados: <?= e(number_format($total_rows,0,',','.')) ?> pedidos — Página <?= e($page) ?>/<?= e($total_pages) ?>
    </div>
    <div style="overflow-x:auto">
      <table>
        <thead>
          <tr>
            <th>ID</th><th>Fecha</th><th>Cliente</th><th>CI/RUC</th><th>Estado</th><th>Total Neto</th><th>Obs.</th><th class="right">Acciones</th>
          </tr>
        </thead>
        <tbody>
...
async function anular(id){
  ...
  if (!confirm("¿Seguro que desea anular el pedido #"+id+"?")) return;

  try{
    const body = new URLSearchParams();
    body.append('id_pedido', id);
    body.append('motivo', motivo || '');

    const res = await fetch('pedido_anular.php', {
      method: 'POST',
    ...
    showToast(d.msg || d.message || (d.success ? 'Ok' : 'Error'));
