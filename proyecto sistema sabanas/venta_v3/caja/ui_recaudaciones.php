<?php
// /caja/recaudaciones.php — UI: Consolidador de Recaudación (opción A: ocultar ya incluidas)
session_start();
...
$w = ["cs.estado='Cerrada'"];
if ($desde !== '') {
  $w[] = "cs.fecha_cierre >= $" . (count($params) + 1);
...
if ($suc   !== '') {
  $w[] = "c.id_sucursal = $" . (count($params) + 1);
  $params[] = $suc;
}

$whereSql = implode(' AND ', $w);

// Traer sesiones cerradas NO incluidas ya en recaudación + totales por medio desde movimiento_caja
$sql = "
SELECT
  cs.id_caja_sesion,
  cs.id_caja,
...
LEFT JOIN public.recaudacion_detalle rd ON rd.id_caja_sesion = cs.id_caja_sesion   -- << NUEVO
WHERE $whereSql
  AND rd.id_caja_sesion IS NULL                                                     -- << NUEVO (excluir ya incluidas)
...
// Totales globales de lo listado
  ...
  <div class="container">
    <div class="head">
      <div>
        ...
        <div class="muted">Seleccioná sesiones <strong>cerradas</strong> para generar una recaudación (quedará <em>Pendiente</em>).</div>
      </div>
      <div class="actions">
    ...
    <div class="card">
      <form method="get" class="grid cols-4" style="align-items:end;">
        ...
        <div>
          ...
          <a class="btn ghost"
            href="/TALLER DE ANALISIS Y PROGRAMACIÓN I/proyecto sistema sabanas/venta_v3/caja/ui_recaudaciones.php?desde=<?= date('Y-m-d') ?>&hasta=<?= date('Y-m-d') ?>">
            Hoy
          </a>
        </div>
      </form>
    ...
    <!-- Tabla de sesiones cerradas (ya no listamos las incluidas) -->
    <div class="card">
      <form id="formRec" onsubmit="return false;">
        <table>
          ...
          <tbody>
            <?php if (empty($rows)): ?>
              <tr>
                <td colspan="10" class="muted">No hay sesiones cerradas disponibles en el rango.</td>
              </tr>
            <?php else: ?>
  ...
  <script>
    (function() {
      ...
      function resumen() {
        ...
        if (sel === 0) {
          resume.textContent = "Ninguna sesión seleccionada.";
          return;
        }
      ...
      async function generar() {
        ...
        if (ses.length === 0) {
          msg.textContent = "Seleccioná al menos una sesión.";
          return;
        }

        msg.textContent = "Generando recaudación...";
        try {
          const res = await fetch('/TALLER DE ANALISIS Y PROGRAMACIÓN I/proyecto sistema sabanas/venta_v3/caja/recaudacion_crear.php', {
            method: 'POST',
            headers: {
      ...
      $("#btnGen")?.addEventListener('click', (ev) => {
        ev.preventDefault();
        generar();
      });
    })();
