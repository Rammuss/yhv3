...
<head>
  ...
  <style>
    ...
    .layout {
      display: grid;
      grid-template-columns: minmax(320px, 2.4fr) minmax(360px, 3fr);
      gap: 16px;
      align-items: start;
    }
...
<body>
  ...
  <div class="layout">
    ...
    <div class="card" id="panel_detalle">
      ...
      <div id="detalle_contenido" style="display:none">
        ...
        <div class="inline-form" style="margin-top:8px">
          <div>
            <label>Producto fraccionado</label>
            <select id="new_ins_prod" onchange="updateInsumoStockHint()"></select>
            <div class="muted" id="new_ins_stock">Stock: -</div>
            <div class="muted" id="new_ins_unidad">Unidad: -</div>
          </div>
          <div>
            <label>Cantidad usada</label>
            <input type="number" step="0.0001" min="0.0001" id="new_ins_cant" value="1">
          </div>
          <div>
  ...
  <script>
    ...
    const STORAGE_SOL_SELECCION = 'otSolicitudesSeleccionadas';
    let seleccionSolicitudPorOT = {};

    try {
      seleccionSolicitudPorOT = JSON.parse(localStorage.getItem(STORAGE_SOL_SELECCION)) || {};
    } catch (e) {
      seleccionSolicitudPorOT = {};
    }

    function guardarSelecciones() {
      localStorage.setItem(STORAGE_SOL_SELECCION, JSON.stringify(seleccionSolicitudPorOT));
    }

    function setSolicitudSeleccionadaOT(otId, value) {
      if (!otId) return;
      if (value === '') {
        delete seleccionSolicitudPorOT[otId];
      } else {
        seleccionSolicitudPorOT[otId] = value;
      }
      guardarSelecciones();
    }
    ...
    async function loadCatalogos() {
      try {
        ...
        catalogoInsumos = (data.insumos || []).map(p => ({
          ...p,
          es_fraccion: normalizarBoolean(p.es_fraccion)
        }));
    ...
    function updateItemHint() {
      ...
      const sel = $('new_item_prod');
      if (!hint || !sel || sel.options.length === 0) {
    ...
    function renderSelectInsumos() {
      ...
      const fraccionados = catalogoInsumos.filter(p => normalizarBoolean(p.es_fraccion));
      fraccionados.forEach(p => {
        const opt = document.createElement('option');
        opt.value = p.id_producto;
        opt.textContent = `${p.nombre}${p.unidad_base ? ' (' + p.unidad_base + ')' : ''}`;
        opt.dataset.stock = p.stock_actual ?? '';
        opt.dataset.unidad = p.unidad_base ?? '';
        opt.dataset.factor = p.factor_equivalencia ?? '';
        sel.appendChild(opt);
      });
      ...
      if (fraccionados.length === 0) {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'No hay fraccionados disponibles';
        sel.appendChild(opt);
        sel.disabled = true;
      } else {
        sel.disabled = false;
    ...
    function updateInsumoStockHint() {
      const sel = $('new_ins_prod');
      const stock = $('new_ins_stock');
      const unidad = $('new_ins_unidad');

      if (!sel || sel.options.length === 0 || !sel.value) {
        if (stock) stock.textContent = 'Stock: -';
        if (unidad) unidad.textContent = 'Unidad: -';
        return;
      }

      const opt = sel.selectedOptions[0];
      const valorStock = opt.dataset.stock ?? '-';
      const valorUnidad = opt.dataset.unidad ?? '';

      if (stock) stock.innerHTML = `Stock: <strong>${valorStock || '-'}</strong>`;
      if (unidad) unidad.innerHTML = `Unidad: <strong>${valorUnidad || '—'}</strong>`;
    }
    ...
    function renderInsumos() {
      ...
      insumos.forEach(ins => {
        ...
        const nombre = prod ? prod.nombre : (ins.descripcion || `#${ins.id_producto}`);
        const unidad = prod ? (prod.unidad_base || '—') : (ins.unidad_base || '—');
        const stock = prod && prod.stock_actual !== undefined ? prod.stock_actual : '—';
        const tr = document.createElement('tr');
        tr.innerHTML = `
      <td>${ins.item_nro}</td>
      <td>${nombre}</td>
      <td>${unidad}</td>
      <td>${stock}</td>
      <td><input type="number" step="0.0001" min="0.0001" value="${ins.cantidad}" onchange="editInsumo(${ins.item_nro},'cantidad',this.value)"></td>
    ...
    function addItem() {
      if (!otSeleccionada) {
        showToast('Seleccioná una OT primero', 'error');
        return;
      }
      const sel = $('new_item_prod');
      ...
      if (!opt) {
        showToast('Seleccioná un servicio o promoción', 'error');
        return;
      }
    ...
    function addInsumo() {
      ...
      const sel = $('new_ins_prod');
      if (!sel || sel.options.length === 0 || !sel.value) {
        showToast('No hay insumos fraccionados disponibles', 'error');
        return;
      }
      const id = Number(sel.value);
      const cant = Number($('new_ins_cant').value || 0);
      if (!cant || cant <= 0) {
        showToast('La cantidad usada debe ser mayor a cero', 'error');
        return;
      }
    ...
    async function crearFromReserva(idManual) {
      ...
      if (id_reserva <= 0) {
        showToast('Indicá el ID de reserva', 'error');
        return;
      }
      try {
        const res = await api('create_from_reserva', {
          id_reserva
        });
    ...
    async function buscarReservas() {
      try {
        ...
        const data = await api('search_reservas', {
          q,
          fecha,
          estado
        });
    ...
    async function changeProfesional() {
      ...
      try {
        await api('update_ot_prof', {
          id_ot: otSeleccionada.id_ot,
          id_profesional: id_prof ? Number(id_prof) : null
        });
    ...
    async function loadSolicitudesCliente() {
      ...
      const otId = otSeleccionada.id_ot;
      const seleccionPrev = seleccionSolicitudPorOT[otId] ?? solicitudSeleccionada ?? '';

      try {
        const data = await api('list_solicitudes_cliente', {
          id_ot: otId
        });
        solicitudesCliente = data.rows || [];

        if (seleccionPrev && !solicitudesCliente.some(sol => String(sol.id_solicitud) === String(seleccionPrev))) {
          solicitudSeleccionada = '';
          setSolicitudSeleccionadaOT(otId, '');
        } else {
          solicitudSeleccionada = seleccionPrev ? String(seleccionPrev) : '';
          setSolicitudSeleccionadaOT(otId, solicitudSeleccionada);
        }
  ...
  `;
      noneLabel.querySelector('input').addEventListener('change', () => {
        solicitudSeleccionada = '';
        if (otSeleccionada) setSolicitudSeleccionadaOT(otSeleccionada.id_ot, '');
        updatePedidoControls();
      });
    ...
    `;
        card.querySelector('input').addEventListener('change', () => {
          ...
          if (otSeleccionada) setSolicitudSeleccionadaOT(otSeleccionada.id_ot, solicitudSeleccionada);
          updatePedidoControls();
        });
