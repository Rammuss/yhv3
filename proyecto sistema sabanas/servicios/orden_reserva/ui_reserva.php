      ...
      case 'create_reserva': {
          ...
          ";
            ...
            if (!$rp || pg_num_rows($rp) === 0) {
              ...
              json_error('No hay profesional libre en ese rango');
            }
        ...
        ";
          ...
          if (!$rCab) {
            pg_query($conn, 'ROLLBACK');
            json_error('No se pudo crear la reserva (cabecera)');
          }
        ...
        ";
          foreach ($snap_items as $it) {
            ...
            if (!$ok) {
              pg_query($conn, 'ROLLBACK');
              json_error('Error al insertar detalle');
            }
          }
  ...
  `;
    ...
    async function verSlots() {
      try {
        ...
        // si la fecha es hoy, quitá horarios que ya pasaron
        const hoyISO = new Date().toISOString().slice(0, 10);
        if (fecha === hoyISO) {
          const ahora = new Date();
          ultimoSlots = ultimoSlots.filter(sl => {
            const inicio = new Date(sl.inicio.replace(' ', 'T')); // responde en hora local
            return inicio >= ahora;
          });
        }
